...
/** 
 * Статусы назначения на должности
 */
class dof_workflow_appointments implements dof_workflow
{
    ...
    {
        if ( ! $appointment = $this->dof->storage($this->get_storage())->get($id) )
        ...
        if ( $flag )
        {
            ...
            $select->basetype  = 'record';
            $select->baseid    = $appointment->departmentid;
            $select->personid  = $this->dof->storage('eagreements')->get_field($appointment->eagreementid,'personid');
                ...
                case 'active':
                    ...
                    {//Обновляем все записи, касающиеся данного назначения
                        ...
                        {// если сама мандата в том же статусе, в которое обновляем применение - можем сменить статус
                            $flag = ( $flag AND $this->dof->workflow('aclwarrantagents')->change($record->id,'active') );
                        }
                    }
                    break;
                case 'canceled':
                    ...
                    $counds->departmentid = $appointment->departmentid;
                    ...
                    foreach($oldlist as $key => $record)
                    {//Обновляем все записи, касающиеся данного назначения
                        $flag = ( $flag AND $this->dof->workflow('aclwarrantagents')->change($record->id,'archive') );
                    }
                    break;
                case 'patient':
                case 'vacation':
