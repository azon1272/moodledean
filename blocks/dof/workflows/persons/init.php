...
/** 
 * Статусы для справочника персон
 */
class dof_workflow_persons implements dof_workflow
{
    ...
    {
        if ( $gentype === 'storage' AND $gencode === 'persons' )
        {// Обрабатываем события от своего справочника
            switch ( $eventcode )
            {
     ...
     * а также выбрасывает событие смены статусов 
     * 
    ...
    public function change($id, $status, $opt = null)
    {
        ...
        // Достаем из базы старый объект чтобы увидеть старый статус
        ...
        $obj->id = intval($id);
        $obj->status = $status;
        //  Заносим в историю запись о смене статуса 
        ...
        if ( $this->dof->storage($this->get_storage())->update($obj) )
        {
            ...
            switch ( $status )
            {
                // Событие смены статуса на актиный
                case 'normal' :
                    $this->dof->send_event('workflow', 'persons', 'person_active', intval($id));
                    break;
                // Событие смены статуса на неактивный
                case 'archive' :
                case 'deleted' :
                    $this->dof->send_event('workflow', 'persons', 'person_not_active', intval($id));
                    break;
            }
        }
    }
